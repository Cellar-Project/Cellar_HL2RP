...
if (CLIENT) then
	...
	if not vgui.GetControlTable("ixTooltipRowIcon") then
	...
	function ITEM:PopulateTooltip(tooltip)
		...
		local skill = tooltip:AddRow("skill")
			...
			if self.Info.Distance then
				...
				for i = 1, 4 do
					local mod = self.Info.Distance[i] or 0
...
-- PlayerDeath hook moved to plugin registration file (sh_plugin.lua)
-- to prevent duplicate hook registration when multiple weapon items load.
