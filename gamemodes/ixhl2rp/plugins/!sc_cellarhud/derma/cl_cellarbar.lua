...
function PANEL:Think()
    ...
    if self.notCellarAlive or !self.client:Alive() then
        self:Remove()
    end
...
function PANEL:Remove()
    ...
    self.levelframe:AlphaTo(0, .6, 2.3)

    timer.Simple(3, function() self:SetVisible(false) end)
...
function PANEL:Think()
    if self.notAliveCellar or self.notAlive == true then
