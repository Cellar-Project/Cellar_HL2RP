...
use function array_slice, intval;
...
class Upgrader
{
	...
	public function assertValidUpgradeSteps(Upgrade\AbstractUpgrade $upgrade)
	{
		$reflection = new \ReflectionObject($upgrade);

		$stepsFound = [];

		foreach ($reflection->getMethods() AS $method)
		{
			if (preg_match('/^step[^0-9]/i', $method->name))
			{
				throw new \RuntimeException("Upgrade {$reflection->name} contains unexpected step, {$method->name}");
			}
			else if (preg_match('/^step([0-9]+)$/i', $method->name, $match))
			{
				// Don't need to worry about dupes -- that's a PHP parse error
				$stepsFound[$match[1]] = true;
			}
		}

		for ($i = 1; $stepsFound; $i++)
		{
			if (!isset($stepsFound[$i]))
			{
				throw new \RuntimeException("Upgrade {$reflection->name} could not find expected step number $i (later steps found)");
			}
