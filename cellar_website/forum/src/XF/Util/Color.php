...
class Color
{
	...
	public static function getAverageColor(array $colors, $inputType = self::RGB, $outputType = self::RGB)
	{
		...
		{
			$rgb = self::getRgb($color, $inputType);
		...
		{
			case self::HSL:
				return self::rgbToHsl($avg);

			case self::HEX:
				return self::rgbToHex($avg);
	...
	{
		$rgb = self::getRgb($color, $inputType);
		$lum = self::getRelativeLuminance($rgb);

		return $lum >= $percentage / 100 ? self::LIGHT : self::DARK;
	}
}