...
class Export extends \XF\Service\AbstractService
{
	...
	public function addChangedAsset(string $key, string $path)
	{
		$this->changedAssets[$key] = $path;
	}
	...
	public function exportToXml()
	{
		...
		$assetsNode = $document->createElement('assets');
		$styleNode->appendChild($assetsNode);

		foreach ($this->getExportableAssets() AS $key => $path)
		{
			$assetsNode->appendChild($this->getAssetNode($document, $key, $path));
		}
	...
	protected function getAssetNode(\DOMDocument $document, string $key, string $path): \DOMElement
	{
		$assetNode = $document->createElement('asset');
		$assetNode->setAttribute('key', $key);
		$assetNode->setAttribute('path', $this->changedAssets[$key] ?? $this->stripDataStylesPathPrefix($path));

		return $assetNode;
	}

	public function stripDataStylesPathPrefix(string $abstractedPath): string
	{
