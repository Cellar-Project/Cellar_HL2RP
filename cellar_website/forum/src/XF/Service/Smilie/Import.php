...
use function in_array, strlen, strval;
...
class Import extends \XF\Service\AbstractService
{
	public function importSmilies(array $smilies, array $newCategories, &$errors = [])
	{
		...
		foreach ($newCategories AS $placeholderCategoryId => $newCategory)
		{
			...
			$categoryEnt->display_order = $newCategory['display_order'] ?? 0;
		...
		foreach ($smilies AS $smilieId => $smilie)
		{
			...
			{
				$smilieCatId = $placeholderCategoryMap[$smilieCatId] ?? 0;
			}
	...
	public function getSmilieDataFromDirectory($directory)
	{
		...
		$xfSourceDir = rtrim(\XF::getSourceDirectory(), '/\\');

		$docRoot = \XF::app()->request()->getServer('DOCUMENT_ROOT');
		if ($docRoot && strpos($xfSourceDir, $docRoot) === 0)
		{
			// we have a document root and the XF dir is in it, trust the server value.
			// Ensure that our path is within the document root.
		...
		$blockPaths = [
			$xfSourceDir,
			\XF\Util\File::canonicalizePath($this->app->config('internalDataPath')),
			\XF\Util\File::canonicalizePath($this->app->config('externalDataPath'))
		];
		foreach ($blockPaths AS $blockPath)
		{
			if (strpos($fullPath, $blockPath) === 0)
			{
				throw new \XF\PrintableException(\XF::phrase('invalid_or_unreadable_directory'));
			}
		}
