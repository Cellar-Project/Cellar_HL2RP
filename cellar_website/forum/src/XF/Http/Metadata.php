...
class Metadata
{
	...
	public function getCharset()
	{
		...
		if (function_exists('mb_check_encoding'))
		{
			try
			{
				$isValid = @mb_check_encoding('', $charset);
			}
			catch (\Error $e)
			{
				$isValid = false;
	...
	public function getFavicon()
	{
		...
		$proxyableFavicons = array_filter($favicons, function ($favicon)
		{
			$path = parse_url($favicon, PHP_URL_PATH);
			$extension = File::getFileExtension($path);
			return File::isImageInlineDisplaySafe($extension);
		});

		$favicon = count($proxyableFavicons) ? reset($proxyableFavicons) : reset($favicons);
		return $this->cleanMetadataString($favicon, true);
	}
	...
	public function convertBodyCharset()
	{
		...
		$body = ($newBody ?: preg_replace('/[\x80-\xff]/', '', $body));
