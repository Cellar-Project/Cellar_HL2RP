...
class Style extends AbstractController
{
		...
		if ($this->isPost())
		{
			...
			switch ($upload->getExtension())
			{
				case 'zip':
					...
					$styleArchiveImporter = $this->service('XF:Style\ArchiveImport', $archiveFile);
	...
	public function actionExportArchive(ParameterBag $params)
	{
		...
		if ($this->isPost())
		{
			...
			$styleArchiveExporter = $this->service('XF:Style\ArchiveExport', $style);
		...
		else
		{
			$viewParams = [
				'style' => $style,
				'asArchive' => true,
				'allowedExtensions' => \XF\Service\Style\ArchiveValidator::EXTENSION_WHITELIST
			];
			return $this->view('XF:Style\Export', 'style_export', $viewParams);
		}
	}
