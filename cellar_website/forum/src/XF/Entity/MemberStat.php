 ...
 * @property string $visibility_class
 * @property string $visibility_method
...
class MemberStat extends Entity
{
	public function canView()
	{
		...
		if ($this->visibility_class || $this->visibility_method)
		{
			$class = $this->visibility_class;
			$method = $this->visibility_method;

			$class = $this->app()->extendClass($class);
			if (\XF\Util\Php::validateCallbackPhrased($class, $method))
			{
				$isVisible = call_user_func_array([$class, $method], [$this]);
				if (!$isVisible)
				{
					return false;
				}
	...
	protected function _preSave()
	{
		...
		if ($this->visibility_class || $this->visibility_method)
		{
			if (!\XF\Util\Php::validateCallbackPhrased($this->visibility_class, $this->visibility_method, $error))
			{
				$this->error($error, 'visibility_method');
			}
		}

		if ($this->isUpdate()
			&& $this->isChanged(['criteria', 'sort_order', 'sort_direction', 'callback_class', 'callback_method', 'visibility_callback', 'visibility_method', 'show_value', 'cache_lifetime', 'user_limit'])
		)
		{
	...
	public static function getStructure(Structure $structure)
	{
		...
		$structure->columns = [
			...
			'visibility_class' => ['type' => self::STR, 'maxLength' => 100, 'default' => ''],
			'visibility_method' => ['type' => self::STR, 'maxLength' => 75, 'default' => ''],
