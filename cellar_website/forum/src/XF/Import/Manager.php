...
class Manager
{
	...
	public function isImportRunning(): bool
	{
	...
	public function getImportNotesData(string $context, \XF\Import\Session $session = null): array
	{
		if (!$session)
		{
			$session = $this->getCurrentSession();
		...
		foreach ($session->stepTotals AS $step => $total)
		{
			if (isset($allSteps[$step]))
			{
				$totals[$step] = [
					...
					'total' => $total,
					'time' => $session->stepTime[$step] ?? null
				];
			}
		}
