...
class Twitch
{
	...
	protected static function getUrl($mediaId)
	{
		...
		// twitch requires a parent paramter indicating domain being served from
		// get host but if port is appended to host it won't work so strip that
		$host = \XF::app()->request()->getHost();
		$host = preg_replace('/:\d+$/', '', $host);
		$url .= '&parent=' . rawurlencode($host);
