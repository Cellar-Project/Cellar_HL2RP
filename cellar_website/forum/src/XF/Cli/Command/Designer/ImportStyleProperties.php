...
class ImportStyleProperties extends AbstractImportCommand
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		if (!$returnCode)
		{
			...
			// explicitly run this every import because updated assets impact output
			$style = $this->getStyleByDesignerModeInput($input, $output);

			/** @var \XF\DesignerOutput\StyleProperty $handler */
			$handler = \XF::app()->designerOutput()->getHandler('XF:StyleProperty');
			$handler->rebuildPropertyLessCache($style);
		}
