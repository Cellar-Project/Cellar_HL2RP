...
class User extends AbstractCriteria
{
	...
	protected function _matchUsername(array $data, \XF\Entity\User $user)
	{
		...
		return in_array(utf8_strtolower($user->username), $names, true);
	}
	...
	protected function _matchQuestionsSolvedMin(array $data, \XF\Entity\User $user)
	{
		return ($user->question_solution_count && $user->question_solution_count >= $data['solved']);
	}

	protected function _matchQuestionsSolvedMax(array $data, \XF\Entity\User $user)
	{
	...
	protected function _matchActivitySummaryEnabled(array $data, \XF\Entity\User $user)
	{
		return $user->last_summary_email_date !== null;
	}

	protected function _matchActivitySummaryDisabled(array $data, \XF\Entity\User $user)
	{
		return $user->last_summary_email_date === null;
	}
